...
def main():
    ...
    creators_with_collab = df_main[(df_main['creator_type'] == 'Business') & 
    ...
    creators_with_collab.to_csv(
        'brandWithNonBlankTopCollaboration.csv',
          index=False,
    ...
    def process_collaborations(row):
        ...
        for username in collaborations:
            ...
            if username in all_usernames:
                ...
                if username_type_map.get(username) == 'Content Creator':
    ...
    print(f"After filtering out non-creators collaborations: {len(creators_with_collab)} rows")
    ...
    def filter_by_logo(collaboration_str):
        ...
        for username in usernames:
            ...
            if username in filename_to_classification and filename_to_classification[username] == 'pic':
    ...
    creators_with_collab['top_collaboration_creator_pic'] = creators_with_collab['top_collaboration'].apply(generate_logo_urls)
    
    # Save the final output
    creators_with_collab.to_csv(
        'brand_with_creator_pic.csv',
        index=False,
        quoting=csv.QUOTE_ALL)
    print(f"Successfully created brand_with_creator_pic.csv with {len(creators_with_collab)} rows")

    # Step 5: Generate creator_with_no_brand.csv
    print("Step 5: Generating brand_with_no_creator.csv...")

    # Get all content creators from the main input
    all_content_creators = df_main[df_main['creator_type'] == 'Business'].copy()

    # If 'top_collaboration_brand_logo' doesn't exist, add it as blank
    if 'top_collaboration_creator_pic' not in all_content_creators.columns:
        all_content_creators['top_collaboration_creator_pic'] = ''
    ...
    creators_with_no_brand.to_csv(
        'brand_with_no_creator.csv',
        index=False,
        quoting=csv.QUOTE_ALL)
    print(f"Successfully created brand_with_no_creator.csv with {len(creators_with_no_brand)} rows")
